...
def plot_multiple(square_meters, bedrooms, y_data, y_pred, beta):
    fig = plt.figure(figsize=(12, 8))
    ax = fig.add_subplot(111, projection='3d')

    # 1. Plot Actual Data (Red Dots)
    actual = ax.scatter(square_meters, bedrooms, y_data, color='red', label='Actual Data', s=50)

    # 2. Plot Predicted Data (Blue Crosses)
    predicted = ax.scatter(square_meters, bedrooms, y_pred, color='blue', marker='x', label='Predicted Data', s=50)

    # 3. Draw lines (Residuals)
    for i in range(len(y_data)):
        ax.plot([square_meters[i], square_meters[i]], [bedrooms[i], bedrooms[i]], 
                [y_data[i], y_pred[i]], color='gray', linestyle='--', alpha=0.5)

    # 4. Create the Regression Surface (The Plane)
    x_surf = np.linspace(square_meters.min(), square_meters.max(), 20)
